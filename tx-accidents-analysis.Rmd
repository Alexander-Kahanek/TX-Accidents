...
The goal is to see if there are more accident during days or nights time.
...
top_fifty <- raw %>%
  group_by(City,Distance.mi., Sunrise_Sunset) %>%
  summarise(
    n= n()
  )

day <- top_fifty %>%
  subset(Sunrise_Sunset == "Day") %>%
  arrange(-n)

day <- day[1:15,]

night <- top_fifty %>%
  subset(Sunrise_Sunset == "Night") %>%
  arrange(-n)
night<-night[1:15,]
...
sun<-raw%>%
  filter(City%in% topfifteen$City)

gg <- ggplot(sun,aes(x=City,y=Distance.mi.,fill=Sunrise_Sunset))+
  geom_bar(stat="identity")+
  theme(axis.text.x = element_text(angle=-45, hjust=-0.1))
gg
```

Dase on this plots we can see that accidents in the United States happens most likely during the Day beacuse the roads are busy and according to the National Highway Traffic Safety Administration (NHTSA), most accidents tend to occur between 3 pm and 6 pm. This is often because of the high volume of vehicles on the road.

# Analysis of Weather and Severity
...
#Severity in the city of Denton with a density and a histogram chart

denton <- df %>%
  subset(City == "Denton")

accidents1 <- ggplot(denton, aes(Severity))
accidents1 + geom_density() + labs(x = "Severity", y = "Range")
```
...
Both of the plots show basically the same thing. One of them is a density plot and the other is a histogram.
This histogram is a very easy plot to understand. you can see which serverity is the highest by just looking at the chart.
The denisty plot visualises the distribution of data over a continuous interval or time period.
The plot only uses Denton and how severity each accident is in that city.As you can see in the plot that the severity is averaging around 3.

## How temperature matches with the serverity of eatch county

```{r}
county <- df %>%
  subset(City == "Fort Worth")

ggplot(data =county )+
  geom_point(mapping=aes(Temperature.F., Severity, color = County))

ggplot(data = county)+
  geom_smooth(mapping=aes(x=Temperature.F., y=Severity,))

#pairs(~Severity+Description+Weather_Condition+Temperatur#e.F.,data=county,main="Scatterplot Matrix", cex= .25)
```

Here are three chart showing the same data on how weather can affeact the serverity of the accidents
The point chart shows you two different county and how each county serverity is during each temperature. 
Same goes with the smooth plot, This one is a little easier to see because it only shows one city.
From the look of each map temperature do play a roll in the severity of each accident.
